...
install.packages("shinydashboard", "tmap", "tmaptools", "leaflet", "devtools", "plotly")
...
DOSPlot <- ggplot(DataFinal, aes(Duration.of.Stay)) + 
  geom_point(aes(y = adventure , colour = "Adventure")) + 
  geom_point(aes(y = pleasure , colour = "Pleasure")) +
  geom_point(aes(y = harmony , colour = "Harmony")) +
  geom_point(aes(y = connection , colour = "Connection")) +
...
#rendering the different visualizations
IncomePlot
summary(IncomeTable)
    ...
    tabItem(tabName = "PredictiveAnalytics",
            fluidRow(
              tabBox(title = "Predictive Regresions", side = "right", selected = "Income Regressions", width = 12, height = 500,
                tabPanel("Income Regressions", plotOutput("IncomePlot", width = 10, height = 400, click = NULL)),
                ...
                shinydashboard::box(width = 4, title = "Selectors", height = 350, status = "danger", 
                                    actionButton("RenderRegression", "Render Regression", icon = NULL)))),
#Data Loading
...
output$IncomePlot <- renderPlot({
  IncomePlot[input$RenderRegression]})
}
